# Complete script for radar-only classification using EfficientNet-B0 with ImageNet pretrained weights
...
# ==== Configuration ====
base_dir = "/nfs/home/noualb20/Sorted_3Radar_BEV_PT224"
output_dir = "/nfs/home/noualb20/111CleanSplit_180/Pre_OUTPUT_RdarOnly"
...
radar_means = [0.00721869, 0.00405002]
radar_stds = [0.25071322, 0.23266373]

# ==== Utility Functions ====
def normalize_tensor(x):
    mean = torch.tensor(radar_means, device=x.device).view(1, 2, 1, 1)
    std = torch.tensor(radar_stds, device=x.device).view(1, 2, 1, 1)
...
# ==== CSV Log Setup ====
csv_headers = ['epoch', 'train_loss', 'train_acc', 'train_f1', 'val_loss', 'val_acc', 'val_f1', 'lr']
csv_file_exists = os.path.isfile(csv_log_path)
...
for epoch in range(start_epoch, num_epochs):
    ...
    with open(csv_log_path, 'a') as f:
        f.write(f"{epoch + 1},{train_loss:.4f},{train_acc:.4f},{train_f1:.4f},{val_loss:.4f},{val_acc:.4f},{val_f1:.4f},{optimizer.param_groups[0]['lr']:.6f}\n")
